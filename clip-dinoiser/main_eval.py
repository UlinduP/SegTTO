...
def evaluate(cfg, val_loaders):
    ...
    task_json_paths = cfg.get("task_json_paths", {})

    for key, loader in val_loaders.items():

        logger.info(f"### Validation dataset: {key}")
        json_path = task_json_paths.get(key, None)
        if not json_path:
            raise ValueError(f"No json_path found for task {key}. Please update your config.")

        logger.info(f"Using json_path: {json_path}")
        # Update config with json_path
        cfg.model.json_path = json_path
