...
from ..tpt.tpt_augmentations import random_crop_and_resize, select_confident_samples, feature_update
...
class DinoCLIP(nn.Module):
    ...
    def __init__(self, clip_backbone, class_names, vit_arch="vit_base", vit_patch_size=16, enc_type_feats="k",
                 gamma=0.2, delta=0.99, apply_found=False, json_path=None):
        ...
        if json_path:
            maskclip_cfg["model"]["decode_head"]["json_path"] = json_path
...
class CLIP_DINOiser(DinoCLIP):
    ...
    def __init__(self, clip_backbone, class_names, vit_arch="vit_base", vit_patch_size=16, enc_type_feats="v",
                 feats_idx=-3, gamma=0.2, delta=0.99, in_dim=256, conv_kernel=3, json_path=None):
        super(CLIP_DINOiser, self).__init__(clip_backbone, class_names, vit_arch, vit_patch_size, enc_type_feats, gamma, json_path=json_path)
        ...
        print("CLIP-DINOiser config:", json_path)
    ...
    def forward(self, x: torch.Tensor):
        tpt_augs = True
        if tpt_augs:
            unpreprocessed_image = UNNORMALIZE(x)
            cropped_imgs, cropped_coords = random_crop_and_resize(unpreprocessed_image, num_crops=64) 
            
            preprocessed_crops = torch.stack([NORMALIZE(img) for img in cropped_imgs])    
            preds, corrs, output = self.forward_pass(preprocessed_crops.to("cuda"))
            ...
            # Get the predictions --------------------------------------------------
            output = self.clip_backbone.decode_head.cls_seg(out_feats, unpreprocessed_image, tpt_tunning=True)
            ...
            if self.apply_found:
                ...
                for i in range(65):  
                    instance_preds = preds[i]
                    instance_output = output[i]
                    
                    soft_found = torch.sigmoid(instance_preds.detach())
                    r_soft_found = soft_found.reshape(-1)
                    nb_cls = instance_output.shape[0] 

                    r_hard_found = (r_soft_found > 0.5).float()
                    uncertain = (instance_output.max(dim=0)[0] < self.delta).reshape(-1)

                    instance_output = instance_output.reshape(1, nb_cls, -1)
                    instance_output[:, 0, uncertain & (~r_hard_found.bool())] = 1.0  # Background class

                    updated_outputs.append(instance_output)
                    updated_preds.append(instance_preds)

                updated_outputs = torch.cat(updated_outputs, dim=0)
                updated_preds = torch.cat(updated_preds, dim=0)
                
            if self.apply_found:
                logits, selected_idxes = select_confident_samples(updated_outputs[1:], 0.2)
            else:
                logits, selected_idxes = select_confident_samples(output[1:], 0.2)
                
            updated_feats = feature_update(out_feats, cropped_coords, selected_idxes)
            
            output = self.clip_backbone.decode_head.cls_seg(updated_feats, unpreprocessed_image, tpt_tunning=False)
            
            if self.apply_found:
                # Compute FOUND --------------------------------------------------
                soft_found = torch.sigmoid(preds[0].detach())
                r_soft_found = soft_found.reshape(-1)
                nb_cls = output.shape[1]
                r_hard_found = (r_soft_found > 0.5).float()
                uncertain = (output.max(dim=1)[0] < self.delta).reshape(-1)
