...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, num_templates=None, n_ctx=16, template_init=None, ctx_position='end', learned_cls=False, attribute_embedding=None, attribute_list=None):
        super().__init__()
        ...
        # #####################################################################################################################
        # temp_templates = []
        # for r in template_init:
        #     if r == 'the_video_of_a':
        #         r = 'the {} in a video game.'
        #     elif r == 'a_video_of_a':
        #         r = 'a {} in a video game.'
        #     else:
        #         r = " ".join(r.split("_")) + " {}."
        #     temp_templates.append(r)
        
        # if self.attribute_list is not None:
        #     tokenized_prompts__new = []
        #     embedded_prompts__new = []
        #     for _class in self.attribute_list:
        #         per_attr_embd = []
        #         per_attr_prompt = []
        #         for attribute in _class:
        #             texts = [template.format(attribute) for template in temp_templates] 
        #             per_template_embd = []
        #             per_template_prompt = []
        #             for pp in texts:
        #                 prompt_ = tokenize(pp).to(self.device) # (1, 77)
        #                 with torch.no_grad():
        #                     embedding = clip_model.token_embedding(prompt_).type(dtype) # (1, 77, 768)
        #                 per_template_embd.append(embedding)
        #                 per_template_prompt.append(prompt_)
        #             per_template_embd = torch.stack(per_template_embd, dim=0) # (5, 1, 77, 768)
        #             per_template_prompt = torch.stack(per_template_prompt, dim=0) # (5, 1, 77)
        #             per_attr_embd.append(per_template_embd)
        #             per_attr_prompt.append(per_template_prompt)
        #         per_attr_embd = torch.stack(per_attr_embd, dim=1).float() # (5, 5, 1, 77, 768)
        #         per_attr_prompt = torch.stack(per_attr_prompt, dim=1).float() # (5, 5, 1, 77)
        #         per_attr_embd = per_attr_embd.mean(dim=1) # (5, 1, 77, 768)
        #         per_attr_prompt = per_attr_prompt.mean(dim=1) # (5, 1, 77)
        #         tokenized_prompts__new.append(per_attr_prompt)
        #         embedded_prompts__new.append(per_attr_embd)
        #     tokenized_prompts__new = torch.stack(tokenized_prompts__new, dim=1).squeeze(2) # (5, 2, 77)
        #     embedded_prompts__new = torch.stack(embedded_prompts__new, dim=1).squeeze(2) # (5, 2, 77, 768)
        #     self.tokenized_prompts__new = nn.Parameter(tokenized_prompts__new)
        #     self.embedded_prompts__new = nn.Parameter(embedded_prompts__new)
...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, num_templates, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, attribute_embedding=None, attribute_list=None):
        ...
        self.num_templates = num_templates
        # prompt tuning
        self.prompt_learner = PromptLearner(clip, classnames, num_templates, n_ctx, ctx_init, ctx_position, learned_cls, attribute_embedding, attribute_list)
    ...
    def get_text_features(self):
        ...
        if self.prompt_learner.attribute_list is not None:
            prompts = self.prompt_learner.embedded_prompts__new
            tokenized_prompts = self.prompt_learner.tokenized_prompts__new
        if self.num_templates is not None:
...
def get_coop(clip_arch, classnames, num_templates, device, n_ctx, ctx_init, learned_cls=False, attribute_embedding=None, attribute_list=None):
    ...
    model = ClipTestTimeTuning(device, classnames, num_templates=num_templates, arch=clip_arch,
                            n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, attribute_embedding=attribute_embedding, attribute_list=attribute_list)
